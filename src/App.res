...
@react.component
let make = () => {
  let form = SidebarForm.useForm(~initialInput=TimerValues.initialConfig, ~onSubmit)
  ...
  <>
    ...
    />
    <ThemeModal
      initialValues onFinish=createNewTheme onCancel=closeModal visible={status === IsCreating}
    />
    {switch status {
    | IsEditing(id, values) =>
      <ThemeModal
