include %form(
  ...
  let validators = {
    ...
    time: {
      strategy: OnSubmit,
      validate: ({time}) => Ok(Js.String.replaceByRe(%re("/_/g"), "0", time)),
    },
    theme: None,
  }
)
...
module Storage = {
  ...
  let get = () =>
    Option.map(localStorage |> getItem(key), value => value |> Js.Json.parseExn |> input_decode)
...
let updateTitle = form =>
  handleChange(form.updateTitle, (input: input, title) => {...input, title: title})
...
let updateTime = form =>
  handleChange(form.updateTime, (input: input, time) => {...input, time: time})

let updateTheme = form =>
  handleChange(form.updateTheme, (input: input, theme) => {...input, theme: theme})
